...
void drawGrid() {
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < columns; j++) {
            DrawRectangleLines(j * width / columns, i * height / rows,  width / columns,  height / rows, BLACK);
        }
    }
}
...
void drawTiles() {
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < columns; j++) {
            if (grid[i][j] == 1) {
                DrawRectangle(j * width / columns, i * height / rows, width / columns, height / rows, PURPLE);
            }
        }
    }
}
